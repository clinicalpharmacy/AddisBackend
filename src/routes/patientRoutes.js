...
router.get('/:identifier', authenticateToken, async (req, res) => {
    try {
        ...
        const { data, error } = await db.from('patients').select('id, user_id, full_name, age, gender, is_pregnant, is_lactating, weight, height, created_at, updated_at, diagnosis, allergies, medication_history, vitals, labs, contact_number, address').eq('id', identifier).maybeSingle();
