...
router.get('/plans/patient/:patientId', authenticateToken, async (req, res) => {
    try {
...
router.post('/plans/pharmacy-assistance', authenticateToken, async (req, res) => {
    try {
        ...
        if (!goals || !goals.trim()) {
            return res.status(400).json({ 
                success: false, 
                error: 'Pharmacy assessment (goals) is required' 
            });
        }
        ...
        if (!notes || !notes.trim()) {
            return res.status(400).json({ 
                success: false, 
                error: 'Plan of action (notes) is required' 
            });
        }
        ...
        // Check if user has permission to add plan for this patient
        const userRole = req.user.role;
        if (userRole !== 'admin' && patient.user_id !== userId) {
            // Check for shared access
            const twentyFourHoursAgo = new Date(Date.now() - 24 * 60 * 60 * 1000).toISOString();
            const { data: accessGrant } = await db
                .from('access_requests')
                .select('id')
            ...
            if (!accessGrant) {
                return res.status(403).json({ 
                    success: false, 
                    error: 'Access denied to add plan for this patient' 
                });
            }
        }
        ...
        if (error) {
            console.error('Database error saving plan:', error);
            return res.status(500).json({ 
                success: false, 
                error: error.message 
            });
...
router.put('/plans/:planId', authenticateToken, async (req, res) => {
    try {
        ...
        // First check if plan exists and user has permission
        const { data: existingPlan, error: fetchError } = await db
            .from('pharmacy_assistance_plans')
            .select('user_id')
